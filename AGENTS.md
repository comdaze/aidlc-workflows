...
- **Hooks** (`hooks/`) — 14 framework hooks, all prefixed `aidlc-*.ts`, covering audit emission, sensor dispatch, runtime-graph compile, session lifecycle, state validation, subagent tracking, statusline rendering, human-presence mint, forwarding-loop enforcement, reviewer read-scope enforcement, code-generation plan-approval enforcement, and direct state-transition enforcement
