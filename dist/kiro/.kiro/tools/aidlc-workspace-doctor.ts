...
// --doctor render loop only prints `fix` on a FAILED row). This keeps a plain
// `/aidlc --doctor` byte-stable for CI while surfacing manifest drift for teams.
//
...
export function workspaceManifestChecks(projectDir: string): DoctorCheck[] {
  ...
  try {
    const proc = Bun.spawnSync({
      cmd: ["git", "-C", projectDir, "status", "--porcelain", "--", "aidlc"],
      stdout: "pipe",
      stderr: "pipe",
    });
    if (proc.exitCode !== 0) {
  ...
  if (existsSync(reposManifestPath)) {
    ...
    if (manifestNames) {
      ...
      const expected = [...manifestNames].sort().map((n) => `/${n}/`);
      if (b === -1 || e === -1 || e < b) {
        results.push({
          pass: true,
          label:
        ...
        results.push({
          pass: true,
          label: inSync
            ? `Workspace .gitignore: managed block matches repos.json (${expected.length} child dir(s))`
