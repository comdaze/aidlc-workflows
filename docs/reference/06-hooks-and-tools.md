...
| `plan-approval-guard.ts` | PreToolUse | Project-wide (settings.json) | `Task` | **Flow-altering.** Enforce code-generation's plan-before-generation ordering (stage Steps 2-4) deterministically: while the workflow's Current Stage is code-generation (and Construction autonomy is not granted), a Task dispatch targeting `aidlc-developer-agent` is refused (exit 2 + a redirecting stderr reason) unless every unit named in the dispatch prompt (any unit, when none is named) has a non-empty `code-generation-plan.md` on disk AND its Plan Approval question records the explicit "Approve Plan" response. Each refusal emits `PLAN_APPROVAL_BLOCKED`. Fail-open on every ambiguity; `AIDLC_DISABLE_PLAN_APPROVAL_GUARD=1` disables enforcement |
...
**Decision.** The guard acts only when the workflow's `Current Stage` is code-generation, Construction autonomy is not `autonomous`, and the tool call is a `Task` dispatch whose `subagent_type` is `aidlc-developer-agent`. It resolves the workflow's known units (the compiled bolt DAG plus the on-disk `construction/<unit>/` dirs), then requires every unit named in the dispatch prompt - any unit, when the prompt names none - to have BOTH a non-empty `code-generation-plan.md` on disk AND an explicit "Approve Plan" answer on the Plan Approval question in `code-generation-questions.md`. Blank tags, "Request Changes", and unrelated answered questions do not authorize generation. The decision (`evaluatePlanApprovalDispatch`, an exported pure function pinned by `t248`) blocks with **exit 2 + a redirecting stderr reason** naming the missing evidence and the stage steps that produce it, and emits a `PLAN_APPROVAL_BLOCKED` audit row (Tool, Target, Stage, Unit).
