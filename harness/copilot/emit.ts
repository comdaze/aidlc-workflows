...
// SessionEnd fires on the CLI only (VS Code local chat parses but never
// executes it — parser-verified); the adapter's session-start reconcile
// covers that surface, so registering it is pure gain, never a dead hook.
// ---------------------------------------------------------------------------
const HOOK_WIRING: Array<{ event: string; target: string; timeoutSec: number }> = [
  ...
  { event: "SessionEnd", target: "session-end", timeoutSec: 30 },
