...
// instead of rendering it through the harness's native question mechanism.
...
describe("t250 (smoke) ```question fence is a SPEC to render, never echoed to chat", () => {
  ...
  for (const harness of HARNESSES) {
    describe(`harness annex: ${harness}`, () => {
      ...
      test("enumerates the structured-question sites the rule applies to", () => {
        const t = read(annexPath(harness));
        // The five sites the fix listed. Pin all five gate names: they are the
        // contract's teeth; a copy-edit may reword the surrounding prose, but
        // dropping a site name means a gate silently fell outside the rule.
      ...
      test("preserves the self-reference carve-out: illustrative example fences survive", () => {
        ...
        const fenceCount = t.split(FENCE).length - 1;
  ...
  test("Claude annex frames the fence as INPUT to the AskUserQuestion tool", () => {
    ...
    // Claude is the one harness whose native mechanism is a tool call; the
    // load-bearing framing from its `## Never echo the spec` section.
  ...
  describe("harness-neutral protocol (stage-protocol.md) carries the same contract", () => {
    ...
    test("keeps the neutral 'spec in, native prompt out' framing for all harnesses", () => {
      const t = read(PROTOCOL);
      expect(/spec in, native prompt out/i.test(t)).toBe(true);
    });
