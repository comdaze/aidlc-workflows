...
//   - W1 (uncommitted records under aidlc/) runs in any git workspace.
...
//     install's report stays byte-stable (no repos.json → exactly the W1 row).
...
describe("t256 workspace-doctor - advisory manifest rows", () => {
  test("no repos.json → only the W1 records row, and it is advisory", () => {
    ...
    // Byte-stable single-repo report: exactly one row (W1), no W2/W3.
